...
plt.savefig("Country_GDP_Bar.png")
plt.show()
...
plt.xticks(rotation=90)
plt.savefig("Country_LEABY_Bar.png")
plt.show()
...
#They do not look similar.  While there is vast divergence in GDP, with the exception of Zimbabwe,
#there is less variation in mean life expectancy among the countries.
...
plt.savefig("Country_GDP_Violin.png")
plt.show()
...
plt.xticks(rotation=90)
#note: plt.xticks("rotation=90") does not work - see incorrect direction in step 4
plt.ylabel('GDP in Trillions of U.S. Dollars')
plt.savefig("Country_GDP_Year_Bar.png")
plt.show()
...
#plt.ylabel('Life expectancy at birth in years') - also works
plt.savefig("Country_LEABY_Year_Bar.png")
plt.show()
...
#While China has the most change in GDP over the reported period, Zimbabwe has the greatest variance in life expectancy
#The years from roughly 2005-2010 appear to have the overall greatest changes among both visualizations
#Based upon what is visible in the chart, Chile has had the most steady GDP level over the measured period, while
# still showing generally sustained growth.  However, Zimbabwe's GDP is so small as to be invisible on the chart - we 
# would need to zoom in to see it better (looking at the raw data, Zimbabwe does actually have the least growth)
#The countries compare well in life expectancy terms, but not as well on a GDP basis
#There does appear to be a correlation between GDP and life expectancy - as the economy of each country has expanded or
# contracted over time, life expectancy in that country has increased or decreased
#There are two possibilities that come to mind, in chicken vs. egg fashion.  If correlation can be interpreted as
# causation: 1) GDP drives life expectancy: a larger economy means a richer population able to afford higher-quality 
# food, healthcare and medication; that, along with expanded government services funded by a larger tax base, could increase
# lifespan.  Or 2) Life expectancy drives GDP: with a healthier population, and with more productive years per capita,
# a greater number of overall productive hours creates greater labor capacity/output and lays the foundation for a 
# growing economy.  However, since the data are for life expectancy at birth, scenario 1 is more likely. Of course,
# Zimbabwe's life expectancy may be a product of factors completely unrelated to GDP.
...
plt.savefig("FacetPlots_LEABY_Country_Year_Scatters.png")
plt.show()
...
#While remaining lowest overall, Zimbabwe moves the most along the Y axis (life expectancy) over time
#The changes in life expactancy in Zimbabwe are surprising in their significance, given the relatively small total
# growth in their economy during the same period
#Relatively easy to read, though referring back to the legend to determine which dots belong to which country is a
# distraction.  A line plot may make following the data easier.
...
plt.savefig("FacetPlots_LEABY_Year_Country_Line.png")
plt.show()
...
plt.savefig("FacetPlots_GDP_Year_Country_Line.png")
plt.show()
#Was unclear if instructions truly wanted us to change the variable for the X axis, as these grids were specifically
# to show GDP change over time....
...
#Why does Zimbabwe have such low overall life expectancy, and what are the factors that have allowed it to improve
# between the mid-2000's and the present?
